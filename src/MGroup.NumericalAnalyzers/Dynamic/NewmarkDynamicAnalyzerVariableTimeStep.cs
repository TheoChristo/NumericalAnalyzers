...
{
	public class NewmarkDynamicAnalyzerVariableTimeStep : INonLinearParentAnalyzer, IStepwiseAnalyzer
	{
		private const string TIME = TransientLiterals.TIME;
		private const string CURRENTTIMESTEP = "Current timestep";
		private const string CURRENTVARIABLETIMESTEP = "Current variable timestep";
		...
		private NewmarkDynamicAnalyzerVariableTimeStep(IAlgebraicModel algebraicModel, ITransientAnalysisProvider provider,
			IChildAnalyzer childAnalyzer, double timeStep, double totalTime, double alpha, double delta, int currentStep, bool calculateInitialDerivativeVectors)
		{
			...
			this.calculateInitialDerivativeVectors = calculateInitialDerivativeVectors;
			this.analysisStatistics = new List<IterativeStatistics>(); //Enumerable.Range(0, Steps).Select(x => new IterativeStatistics() { AlgorithmName = "Newmark dynamic analyzer" }).ToArray();
		}
		...
		private void SolveForInitialConditions()
		{
			...
			var rhsVector = provider.GetRhs(currentTime);
		...
		private void SolveCurrentTimestep()
		{
			...
			AddHigherOrderContributions(currentTime);
			IGlobalVector rhsVector = provider.GetRhs(currentTime);
			ChildAnalyzer.CurrentAnalysisLinearSystemRhs.CopyFrom(rhsVector);
			...
			analysisStatistics.Add(ChildAnalyzer.AnalysisStatistics);
			end = DateTime.Now;
		...
		GenericAnalyzerState CreateState()
		{
			currentState = new GenericAnalyzerState(this,
				...
				new[]
				{
					...
					(CURRENTVARIABLETIMESTEP, timeStep),
				});
		...
		public class Builder
		{
			...
			public NewmarkDynamicAnalyzerVariableTimeStep Build()
				=> new NewmarkDynamicAnalyzerVariableTimeStep(algebraicModel, provider, childAnalyzer, timeStep, totalTime, beta, gamma, currentStep, calculateInitialDerivativeVectors);
		}
	}
}
